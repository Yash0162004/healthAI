...
st.image(logo, caption='Multiple Disease Prediction System', use_column_width=True)
...
if selected == 'Disease Prediction':
    ...
    if st.button('Predict'):
        ...
        # Tabs for Description and Precautions
...
if selected == 'Diabetes Prediction':
    ...
    with col2:
        Insulin = st.number_input("Insulin Value", min_value=0)
    with col3:
    ...
    if st.button("Diabetes Test Result"):
        ...
        if diabetes_prediction[0] == 1:
            ...
            st.image(image, caption='Diabetic')
        else:
            diabetes_dig = "Congratulations, You are not Diabetic!"
            image = Image.open('negative.jpg')
            st.image(image, caption='Not Diabetic')
...
if selected == 'Heart disease Prediction':
    # Display the header and image for heart disease prediction
    ...
    # Define columns for input grouping
    col1, col2, col3 = st.columns(3)
    ...
    with col2:
        chol = st.number_input("Serum Cholesterol", min_value=100, max_value=500)
    with col3:
        fbs = st.checkbox("Fasting Blood Sugar > 120 mg/dl", value=False)
        fbs = 1 if fbs else 0
    ...
    with col3:
        exang = st.checkbox("Exercise Induced Angina", value=False)
        exang = 1 if exang else 0

    with col1:
        oldpeak = st.number_input("ST Depression Induced by Exercise", min_value=0.0, max_value=10.0, step=0.1)
    with col2:
        slope = st.selectbox("Peak Exercise ST Segment", options=["upsloping", "flat", "downsloping"])
    ...
    if st.button("Heart Test Result"):
        ...
        # Display result and corresponding image
        if heart_prediction[0] == 1:
            heart_dig = "We are really sorry to say, but it seems like you have Heart Disease."
            result_img = Image.open('positive.jpg')
        else:
            heart_dig = "Congratulations, you don't have Heart Disease."
            result_img = Image.open('negative.jpg')

        st.image(result_img, caption=heart_dig, use_column_width=True)
